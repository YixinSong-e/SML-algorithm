...
fun shai(curr:IntInf.int,last:IntInf.int):int
= if(curr * curr > last) then 1 
else let
	val k = last mod curr
	in
	if(k = 0) then 0 else shai(curr+1,last)
	end;
val bo = shai(2,n);
if(bo = 1) then printString("True") else printString("False");
(*****End*****)
